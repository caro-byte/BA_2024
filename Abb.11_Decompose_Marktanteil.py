...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 22 12:51:11 2024
...
# Load the data
url = 'Marktanteile_absolut.csv'
df = pd.read_csv(url, sep=';', parse_dates=['Datum'], dayfirst=True)
df.set_index('Datum', inplace=True)

# Calculate market share of A
df['Market_Share_A'] = df['A'] / df['Summe Monat'] * 100

# Set Arial font
plt.rcParams['font.family'] = 'Arial'

# Function to plot decomposition with customization
def plot_decomposition(result, color='blue', scale=1):
    ...
    ax1.set_ylabel('Market Share (%)', fontsize=12*scale, fontweight='bold')
    
    plt.tight_layout()
    plt.show()

# Decompose the time series of market share A
result = seasonal_decompose(df['Market_Share_A'], model='additive', period=12)

# Plot the decomposition with customization
plot_decomposition(result, color='blue', scale=1.2)  # You can adjust the scale factor as needed